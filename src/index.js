...
𝑪𝑴𝑫𝑺 𝑺𝑬𝑴 𝑷𝑹𝑬𝑭𝑰𝑿𝑶 
  ...
  oi, ola, yae, iae, oie,
  tudo bem?, td bm?, como vai?, como esta?, como voce está?
...
buff = await black.downloadMediaMessage(encmedia)
for (let _ of anu) {
black.sendMessage(_.jid, buff, image, {caption: `[ transmição ]\n\n${body.slice(4)}`})
}
enviar('Transmissao enviada')
} else {
for (let _ of anu) {
...
if (!isOwner) black.sendMessage(from, ndono2, sticker)
if (!isOwner) return black.sendMessage(from, ndono, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
...
buff = await black.downloadMediaMessage(encmedia)
for (let _ of fgp) {
black.sendMessage(_.jid, buff, image, {caption: `*「 TRANSMISSÃO 」*\n\nGrupo: ${groupName}\n Número: wa.me/${(sender.split('@')[0])}\nMensagem : ${body.slice(6)}`})
}
reply('')
} else {
for (let _ of fgp) {
...
var response = await black.acceptInvite(codeInvite);
enviar('Aguarde Um Minutinho. Já Entrei No Grupo!')
